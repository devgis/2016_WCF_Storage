    ...
    public partial class frmSelectDateTime : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            if (M_Str_object == "Detailed")
            {
                ...
                DataSet ds = null;
                ds = PublicDim.client.SellStockDetailed(G_Str_tradeCode, dtpStar.Value, dtpEnd.Value, "tb_sell_detailed");
                sellDetailed.dgvSaleStockList.DataSource = ds.Tables[0].DefaultView;
                ds = PublicDim.client.SellStockDetailed(G_Str_tradeCode, dtpStar.Value, dtpEnd.Value, "tb_resell_detailed");
            ...
            else
            {
                ...
                string P_Str_Code = "";
                ds = PublicDim.client.FindCurrentAccountDate(M_Str_units, dtpStar.Value, dtpEnd.Value);
                //设置表格列
                ...
                if (ds.Tables[0].Rows.Count > 0)   //存在往来关系
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                   {
                        ...
                        {
                            ds_Detailed = PublicDim.client.FindDetailde("tb_warehouse_detailed", ds.Tables[0].Rows[i]["billcode"].ToString());
                            ds_main = PublicDim.client.FindMain("tb_warehouse_main", ds.Tables[0].Rows[i]["billcode"].ToString());
                        ...
                        {
                            ds_Detailed = PublicDim.client.FindDetailde("tb_rewarehouse_detailed", ds.Tables[0].Rows[i]["billcode"].ToString());
                            ds_main = PublicDim.client.FindMain("tb_rewarehouse_main", ds.Tables[0].Rows[i]["billcode"].ToString());
                            //动态设置表格行数(注意:每当增加一些新表格的时,原有表格中最后一行数据,DataGridView控件将其自动清空)
                            G_Int_rows = G_Int_rows + ds_Detailed.Tables[0].Rows.Count;
                            currentAccountBook.dgvCurrentAccountList.RowCount = G_Int_rows;
                            //动态创建新表格后,填充-自动清空的行数据
                        ...
                        {
                            ds_Detailed = PublicDim.client.FindDetailde("tb_sell_detailed", ds.Tables[0].Rows[i]["billcode"].ToString());
                            ds_main = PublicDim.client.FindMain("tb_sell_main", ds.Tables[0].Rows[i]["billcode"].ToString());
                            //动态设置表格行数(注意:每当增加一些新表格的时,原有表格中最后一行数据,DataGridView控件将其自动清空)
                            G_Int_rows = G_Int_rows + ds_Detailed.Tables[0].Rows.Count;
                            currentAccountBook.dgvCurrentAccountList.RowCount = G_Int_rows;
                            //动态创建新表格后,填充-自动清空的行数据
                        ...
                        {
                            ds_Detailed = PublicDim.client.FindDetailde("tb_resell_detailed", ds.Tables[0].Rows[i]["billcode"].ToString());
                            ds_main = PublicDim.client.FindMain("tb_resell_main", ds.Tables[0].Rows[i]["billcode"].ToString());
                            //动态设置表格行数(注意:每当增加一些新表格的时,原有表格中最后一行数据,DataGridView控件将其自动清空)
                            G_Int_rows = G_Int_rows + ds_Detailed.Tables[0].Rows.Count;
                            currentAccountBook.dgvCurrentAccountList.RowCount = G_Int_rows;
                            //动态创建新表格后,填充-自动清空的行数据
