...
namespace EMS.SaleStock
{
    public partial class frmSellStockCost : Form
    {
        ...
        private void frmSellStockCost_Load(object sender, EventArgs e)
        {
            ...
            ds = PublicDim.client.GetAllBill("tb_sell_main");
            //设置初始化表格
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                //设置清单摘要
                ds_Detailed = PublicDim.client.GetDetailedkByBillCode(ds.Tables[0].Rows[i]["billcode"].ToString(), "tb_sell_detailed");
                ...
                ds_Stock = PublicDim.client.GetStockByTradeCode(ds_Detailed.Tables[0].Rows[0]["tradecode"].ToString(), "tb_Stock");
        ...
        private void tlbtnFind_Click(object sender, EventArgs e)
        {
            ...
            ds = PublicDim.client.FindSellStock(dtpStar.Value, dtpEnd.Value);
            //设置初始化表格
            dgvStockList.RowCount = ds.Tables[0].Rows.Count + 1;
            dgvStockList.ColumnCount = ds.Tables[0].Columns.Count - 1;
