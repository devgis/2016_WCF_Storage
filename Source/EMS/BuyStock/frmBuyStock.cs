...
namespace EMS.BuyStock
{
    public partial class frmBuyStock : Form
    {
        ...
        DBSR.cBillInfo billinfo = new DBSR.cBillInfo();//创建cBillInfo类的对象
        DBSR.cCurrentAccount currentAccount = new DBSR.cCurrentAccount();//创建cCurrentAccount类的对象
        ...
        private void frmBuyStock_Load(object sender, EventArgs e)
        {
            ...
            ds = PublicDim.client.GetAllBill("tb_warehouse_main");//获取所有进货单信息
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            //执行添加操作
            PublicDim.client.AddTableMainWarehouse(billinfo, "tb_warehouse_main");
            //向进货（明细表）中录入商品单据信息
            for (int i = 0; i < dgvStockList.RowCount - 1; i++)
            {
                ...
                //执行多行录入数据（添加到明细表中）
                PublicDim.client.AddTableDetailedWarehouse(billinfo, "tb_warehouse_detailed");
                //更改库存数量和加权平均价格
                DataSet ds = null;//创建数据集对象
                ...
                int d = PublicDim.client.UpdateStock_QtyAndAveragerprice(stockinfo);//执行更新操作
            }
