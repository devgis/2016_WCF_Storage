    ...
    public partial class frmCheckStock : Form
    {
        ...
        private void tlBtnFind_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    dgvStockList.DataSource = PublicDim.client.GetAllStock("tb_stock").Tables[0].DefaultView;//获取所有商品信息并显示
            ...
            if (tlCmbStockType.Text == "商品产地")//按商品产地查询
            {
                ...
                ds = PublicDim.client.FindStockByProduce(stockinfo, "tb_stock");//根据商品产地查询信息
                dgvStockList.DataSource = ds.Tables[0].DefaultView;//显示查询到的信息
            }
            else//按商品名称查询
            {
                stockinfo.FullName = tlTxtFindStock.Text;//记录商品名称
                ds = PublicDim.client.FindStockByFullName(stockinfo, "tb_stock");//根据商品名称查询信息
                dgvStockList.DataSource = ds.Tables[0].DefaultView;//显示查询到的信息
            }
            this.SetdgvStockListHeadText();//设置DataGridView控件列标题
        }
        ...
        {
            dgvStockList.DataSource = PublicDim.client.GetAllStock("tb_stock").Tables[0].DefaultView;//显示所有商品信息
            this.SetdgvStockListHeadText();//设置DataGridView控件列标题
        }
        ...
        private void tlbtnCheckStock_Click(object sender, EventArgs e)
        {
            ...
            int d = PublicDim.client.CheckStock(stockinfo);//执行库存盘点操作
